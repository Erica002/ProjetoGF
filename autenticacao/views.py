    ...
    def post(self, request):
        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                if len(password) < 8:
                    messages.error(
                        request, "A senha precisa ter pelo menos 8 caracteres"
                    )
                    return render(
                        request, "autenticacao/cadastro-usuario.html", context
                    )
...
class ValidarUsuarioView(View):
    def post(self, request):
        ...
        if not str(username).isalnum():
            return JsonResponse(
                {
                    "error_usuario": "O nome de usuário deve ser composto de apenas letras e/ou números)"
                },
                status=400,
            )
        ...
        if User.objects.filter(username=username).exists():
            return JsonResponse(
                {"error_usuario": "Esse nome de usuário já existe"}, status=409
            )
...
class ValidarEmailView(View):
    def post(self, request):
        ...
        if not validate_email(email):
            return JsonResponse(
                {
                    "error_email": "Formato de email inserido é inválido. Tente novamente."
                },
                status=400,
            )
...
class LoginUsuarioView(View):
    ...
    def post(self, request):
        ...
        if username and password:
            ...
            messages.error(
                request, "Nome de usuário ou senha incorreta. Tente novamente"
            )
            return render(request, "autenticacao/login.html")
